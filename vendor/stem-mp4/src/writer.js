...
class StemMp4Writer {
  ...
  static async write(opts) {
    const {
      ...
      sampleRate = 44100,
      masteringDsp = null,
      stemNames: customStemNames = null,
      artwork = null,
    ...
    const defaultStemNames =
      profile === 'STEMS-2' ? ['Music', 'Vocals'] : ['Drums', 'Bass', 'Other', 'Vocals'];
    const stemNames = Array.isArray(customStemNames) && customStemNames.length === defaultStemNames.length
      ? customStemNames.map((name, index) => String(name || '').trim() || defaultStemNames[index])
      : defaultStemNames;
    ...
    if (artwork) {
      file = await Atoms.addCoverArtworkBuffer(file, artwork, artworkMimeType);
    }
