/**
 * Stem MP4 Reader
 * Load and parse Stem MP4 files with karaoke extensions
 */
...
class StemMp4Reader {
  ...
  /**
   * Load and parse a Stem MP4 file with karaoke extensions
   * @param {string} m4aPath - Path to .stem.mp4 (or legacy .stem.m4a) file
   * @returns {Promise<Object>} Parsed stem data
   */
  static async load(m4aPath) {
    try {
      ...
      const metadata = {
        ...
        releaseDate: mmData.common?.releasedate || mmData.common?.date || mmData.common?.year || '',
        producer: Array.isArray(mmData.common?.producer) ? mmData.common.producer.join('; ') : (mmData.common?.producer || ''),
        label: Array.isArray(mmData.common?.label) ? mmData.common.label.join('; ') : (mmData.common?.label || ''),
        genre: Array.isArray(mmData.common?.genre) ? mmData.common.genre.join('; ') : (mmData.common?.genre || ''),
