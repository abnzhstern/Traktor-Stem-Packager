 ...
 */
function buildNiStemsMetadataBytes(stemNames, masteringDsp = null) {
  // Traktor Pro 4 native generated-stem defaults.
  const colors = ['#FD6C38', '#D232F4', '#00FFAC', '#45DAFD'];
  const stemsMetadata = {
    version: 2,
 ...
 * @returns {Uint8Array} new MP4 bytes
 */
export function addNiStemsMetadataBuffer(uint8, stemNames = null, masteringDsp = null) {
  if (!stemNames) {
    stemNames = ['Drums', 'Bass', 'Other', 'Vocals'];
  }
  const fileBuffer = toUint8Array(uint8);
  const metadataBytes = buildNiStemsMetadataBytes(stemNames, masteringDsp);
...
/** Add JPEG or PNG cover artwork as the standard MP4/iTunes covr atom. */
export function addCoverArtworkBuffer(uint8, artwork, mimeType = 'image/jpeg') {
  const fileBuffer = toUint8Array(uint8);
  const payload = toUint8Array(artwork);
  if (payload.length === 0) return fileBuffer;
  const typeFlag = mimeType === 'image/png' ? 14 : 13;
  return injectAtomToIlstCore(fileBuffer, createAtom('covr', createDataAtom(typeFlag, payload)));
}
